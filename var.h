...
std::vector<std::string> genExpression(const std::vector<std::string> &p) {
  ...
  for (size_t i = 0; i < temp.size(); i++)
    ret.push_back(*(temp.crbegin() + i));
  for (size_t i = 0; i < ret.size(); i++)
    fin.push_back(*(ret.crbegin() + i));
  return fin;
}
...
public:
  ...
  void update() {
    ...
    case Object: {
      ...
      for (std::map<std::string, var>::const_iterator x = ObjectValue.cbegin();
           x != ObjectValue.cend(); x++) {
        if (x->second.needtoRemove == false)
  ...
  const std::string toString_nonconst() const {
    std::string tmp;
    switch (tp) {
    case Null:
      return "null";
    case Int: {
      if (IntValue == (int)IntValue)
        return std::to_string((int)IntValue);
    ...
    case Object: {
      tmp = "{";
      for (std::map<std::string, var>::const_iterator it = ObjectValue.cbegin();
  ...
  const std::string toString() const {
    if (isConst)
      return "const<" + toString_nonconst() + ">";
    else
      return toString_nonconst();
  }
...
const var parse(const std::string &x, bool isc = false, bool forceisc = false) {
  std::string p = clearnull(x);
  if (p == "")
    return var(nullptr, isc);
  try {
    if (isExpression(p) == false)
      throw 0;
  } catch (...) {
    try {
      ...
      if (p.substr(0, 6) == "const<" && p[p.length() - 1] == '>') {
        if (forceisc)
          return parse(p.substr(6, p.length() - 7), isc);
      ...
      if ((p[0] == '{' || p[0] == '[') &&
        ...
        for (size_t i = 1, a = 0; i < p.length() - 1; i++) {
          ...
          if (isobject)
            ret[parse(key).StringValue] = parse(value);
          else
      ...
      if (p[0] == '(' && p[p.length() - 1] == ')')
        return var(Fn_temp(code_split(p.substr(1, p.length() - 2))), isc);
      return var(genExpression(splitExpression(p)), isc);
    }
    if (p.find_first_of('.') == std::string::npos &&
        p.find_first_of('e') == std::string::npos)
      return var(std::stoi(p, 0, 0), isc);
    else
      return var(std::stod(p), isc);
  }
  return var(genExpression(splitExpression(p)), isc);
}
} // namespace Variable
