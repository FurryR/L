...
#include<iostream>
#include<fstream>
#define VERSION_INFO "1.2.0-r_release"
...
void lang_init(){
    ...
    "lpp_unknown_internal_stmt:\"Unknown internal statement.After checking spell,please try again.\"\n"
    ...
    if(arg.size()==0||(uselangfile&&arg.size()==1)||arg[uselangfile]=="--help"||arg[uselangfile]=="help"){
        ...
        while(!s.eof()){
            ...
            for(size_t i=0;i<w.size();i++){
                ...
                f=x.scope;
                if(x.tp==L::L::Return_Type::Throw_Return_Value){
                    //std::cout<<"    "<<w[i]<<" -> "<<x.base.toString()<<std::endl<<"[x]Interpreter throws a(an) "<<x.value.toString()<<" Error."<<std::endl;
                    std::cout<<strmap["lpp_throw"].format({Variable::clearnull(w[i]),x.base.toString(),x.value.toString()})<<std::endl;
                    return 1;
